...
const SmartSearch = ({
  ...
  const RenderProjectList = ({ projectList, type, errorMessage }) => {
    if (projectList && projectList.length > 0) {
      ...
      if (errorMessage !== "") {
        return <NoContentCard text={errorMessage} />;
      } else {
        return <></>;
      }
    }
  ...
  const handleSearch = (searchBy, search) => {
    if (search !== null) {
      if (searchBy === "id") {
        setIdResults(
          projects.filter((project) => project.project_id.includes(search))
        );
      }
      if (searchBy === "title") {
        setTitleResults(
